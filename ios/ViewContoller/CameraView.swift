...
class CameraView: UIView {
  ...
  func writeFrame(_ sampleBuffer: CMSampleBuffer) {
    guard isRecording,
    ...
    // Première frame -> on démarre la session AVAssetWriter
    ...
    // Append tant que l’input est prêt
  ...
  func startRecording() throws {
    ...
    let url = temp.appendingPathComponent("mediapipe-\(UUID().uuidString).mp4")
    outputURL = url
    ...
    DispatchQueue.main.async { [weak self] in
      ...
      if let landmarks = results {
        ...
        swiftDict["additionalData"] = [
          "height": self?.landmarkData.height
            ?? CGFloat(self!.isPortrait ? DefaultConstants.HEIGHT : DefaultConstants.WIDTH),
          "width": self?.landmarkData.width
            ?? CGFloat(self!.isPortrait ? DefaultConstants.WIDTH : DefaultConstants.HEIGHT),

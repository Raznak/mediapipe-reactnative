...
protocol CameraFeedServiceDelegate: AnyObject {
   ...
   */
  func didOutput(
    sampleBuffer: CMSampleBuffer, orientation: UIImage.Orientation, landmarkData: LandmarkData)
...
class CameraFeedService: NSObject {
  ...
  func observeValueForKeyPath(
    _ keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?,
  ...
  func stopRecording() {
    if isReacordingStart {
      ...
      isTimerStart = false
      guard videoInput?.isReadyForMoreMediaData == true, assetWriter!.status != .failed else {
        return
      }
...
private var videoOutputURL: URL = documentDirectory.appendingPathComponent(
...
extension CameraFeedService: AVCaptureVideoDataOutputSampleBufferDelegate {
  ...
  func captureOutput(
    ...
    guard let imageSize = imageBufferSize else {
      imageBufferSize = CGSize(
        width: CVPixelBufferGetWidth(imageBuffer), height: CVPixelBufferGetHeight(imageBuffer))
      return
    }
    ...
    if self.isPoseStarted {
      ...
      let data = LandmarkData(
        height: imageSize.height, width: imageSize.width, frameNumber: frameCount,
        presentationTimeStamp: Double(timestamp.value), frameRate: self.frameRate,
  ...
  func processSampleBuffer(
    imageBuffer: CVImageBuffer, sampleBuffer: CMSampleBuffer, imageSize: CGSize
  ) -> CMSampleBuffer? {
    ...
    let texts = [
      self.eventName, timerRepStrig, "", currentTimestampString,
      " / " + String(self.startTimestamp),
    ]
    ...
    guard
      let newPixelBuffer = renderCIImageToPixelBuffer(
        watermarkedImage,
        imageSize: CGSize(
          width: CVPixelBufferGetWidth(imageBuffer), height: CVPixelBufferGetHeight(imageBuffer)))
    else {
    ...
    CMVideoFormatDescriptionCreateForImageBuffer(
      allocator: kCFAllocatorDefault, imageBuffer: newPixelBuffer,
    ...
    var timingInfo = CMSampleTimingInfo(
      ...
      presentationTimeStamp: presentationTimeStamp, decodeTimeStamp: CMTime.invalid)

    CMSampleBufferCreateForImageBuffer(
      allocator: kCFAllocatorDefault, imageBuffer: newPixelBuffer, dataReady: true,
      makeDataReadyCallback: nil, refcon: nil, formatDescription: formatDescription!,
      sampleTiming: &timingInfo, sampleBufferOut: &newSampleBuffer)
  ...
  func renderCIImageToPixelBuffer(_ image: CIImage, imageSize: CGSize) -> CVPixelBuffer? {
    ...
    CVPixelBufferCreate(
      kCFAllocatorDefault, Int(imageSize.width), Int(imageSize.height), kCVPixelFormatType_32BGRA,
      pixelBufferAttrs as CFDictionary, &pixelBuffer)
  ...
  func generateWatermarkImage(
    imageSize: CGSize, texts: [String], logo: UIImage,
    ...
    let image = renderer.image { context in
      ...
      for (index, text) in texts.enumerated() {
        ...
        if index == 0 {
          textOrigin = CGPoint(
            x: imageSize.width - textSize.width - 21 * widthRatio,
            y: isPortrait ? 42 * heightRatio : 52 * heightRatio)
        ...
        if index == 1 {
          textOrigin = CGPoint(
            x: imageSize.width - 320 * widthRatio,
            y: isPortrait ? 194 * heightRatio : 210 * heightRatio)
          backgroundRect = CGRect(
            x: textOrigin!.x - (37.4 * widthRatio),
            y: textOrigin!.y - 25 * heightRatio,
      ...
      let watermarkPosition = CGPoint(
        x: 10 * widthRatio, y: imageSize.height - watermarkSize.height - 21 * heightRatio)  // Adjust position if needed
    ...
    let ciImage = CIImage(cgImage: cgImage, options: nil).transformed(
      by: CGAffineTransform(scaleX: 1.0 / scale, y: 1.0 / scale))

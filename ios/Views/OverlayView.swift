...
struct OverlayPointConfig {
  let index: Int
  let color: UIColor
  let radius: CGFloat
}

struct OverlayLineConfig {
  let from: Int
  let to: Int
  let color: UIColor
}
...
class OverlayConfiguration {
  var points: [OverlayPointConfig] = []
  var lines: [OverlayLineConfig] = []
}
...
class OverlayView: UIView {
  ...
  func parseOverlayConfig(_ dict: NSDictionary) -> OverlayConfiguration
  ...
  private func drawPoints(from configs: [OverlayPointConfig], using dots: [CGPoint]) {
    for cfg in configs {
      let p = dots[cfg.index]
      let rect = CGRect(
        x: p.x - cfg.radius / 2,
        y: p.y - cfg.radius / 2,
        width: cfg.radius,
  ...
  override func draw(_ rect: CGRect) {
    for poseOverlay in poseOverlays {
      drawLines(from: overlayConfig.lines, using: dots)
      drawPoints(from: overlayConfig.points, using: dots)
  ...
  private func rectAfterApplyingBoundsAdjustment(
    onOverlayBorderRect borderRect: CGRect
  ) -> CGRect {
    ...
    var newRect =
      ...
      .applying(
        CGAffineTransform(
          scaleX: offsetsAndScaleFactor.scaleFactor, y: offsetsAndScaleFactor.scaleFactor)
      )
      .applying(
        CGAffineTransform(
          translationX: offsetsAndScaleFactor.xOffset, y: offsetsAndScaleFactor.yOffset)
      )

    if newRect.origin.x < 0 && newRect.origin.x + newRect.size.width > edgeOffset {
      newRect.size.width = newRect.maxX - edgeOffset
      newRect.origin.x = edgeOffset
    }

    if newRect.origin.y < 0 && newRect.origin.y + newRect.size.height > edgeOffset {
  ...
  func poseOverlays(
    ...
    imageContentMode: UIView.ContentMode,
    andOrientation orientation: UIImage.Orientation, isPortrait: Bool,
    propDictionary: [String: Bool]
  ) -> [PoseOverlay] {
    ...
    for poseLandmarks in landmarks {
      ...
      switch orientation {
      case .left:
        transformedPoseLandmarks = poseLandmarks.map({
      ...
      let dots: [CGPoint] = transformedPoseLandmarks.map({
        CGPoint(
          x: CGFloat($0.x) * originalImageSize.width * offsetsAndScaleFactor.scaleFactor
            + offsetsAndScaleFactor.xOffset,
          y: CGFloat($0.y) * originalImageSize.height * offsetsAndScaleFactor.scaleFactor
            + offsetsAndScaleFactor.yOffset)
      })
